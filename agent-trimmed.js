#!/usr/bin/env node
/**
 * RazzLab Repo Agent (trimmed context)
 * Node >=18 required.
 */
...
const MODEL = args.get("model") || process.env.MODEL || "gpt-5-mini";
const API_KEY = process.env.OPENAI_API_KEY;
// Optional include filter: comma-separated globs like src,api,app
const INCLUDE = (args.get("include") || process.env.AGENT_INCLUDE || "").toString().split(",").map(s => s.trim()).filter(Boolean);
...
// Trimmed limits to avoid rate-limit/size errors
const MAX_FILES = 80;
...
function passesInclude(rel) {
  if (!INCLUDE.length) return true;
  return INCLUDE.some(prefix => rel === prefix || rel.startsWith(prefix + path.sep));
}
...
Output strict JSON with keys: plan, edits[], commands[], notes.
Prefer full 'content' for new/small files; unified 'patch' for large files.`
  };
